...
import "./RiskInspectionProps.css";
...
export default function RiskInspectionProps({ section1, section2, onClick, downloadQuotePdf }) {
          ...
          <div style={{ height: "400px", padding: '25px' }}>
            {section2.blur ? (
              <div className={"quotationpolicySection"}>
                <p>
                 Quotation has been created but Requires Risk Inspection <br /> approval.
                </p>
                <p>
            ...
            {section2.title ? (
              <p style={{ marginBottom: section2.subTitle ? '10px' : '' }}>
                {section2.title}{" "}
                {section2?.tag?.name ? <span style={{ backgroundColor: section2?.tag?.color }}>{section2?.tag?.name}</span> : ""}
              </p>

            ) : (
              ""
            )}
            {section2.subTitle ? <div style={{ marginBottom: '10px' }}>{section2.subTitle}</div> : ''}
            <div
            ...
            >
              <div className={section2.blur ? "blur" : ""}>
                ...
                <div className="totalAmount">
                  <p>Total Amount</p>
                  <p>{currencySign[tripInfo?.currency]}{section2?.totalAmount ?? '-'}</p>
                </div>
                {/* {section1.share.show ? (
              ...
              >
                <p>
                 Quotation has been created but requires Requires Risk Inspection <br /> approval.{" "}
                </p>
                <p>
            ...
            {section2.successBtn.show ? (
              <Button className="successBtn" onClick={() => onClick && onClick()}>{section2.successBtn.text}</Button>
            ) : (
              ""
            )}
            {section2.downloadEditBtn.show ? (
              <div className="setOfbtn">
                <Button className="quoteWhiteBtn" onClick={() => downloadQuotePdf && downloadQuotePdf()}>Download Quote (PDF)</Button>
