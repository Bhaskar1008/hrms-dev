...
import WithoutApproxPolicyFooter from "../MotorFormFooter/PolicyQuoteFooter/WithoutApproxPolicyFooter";
...
export default function PolicyTechnicalControlMotor() {
  ...
  let quotationNumber =
    qoutationFormData?.motorQuotation?.motorPolicyResponse?.data
  ...
  let policyNumber =
    qoutationFormData?.motorQuotation?.motorPolicyResponse?.data
      ?.oonaPolicyResponse?.policyNumber;
  ...
  const QuotationPending = {
    ...
    section2: {
      title: `Policy #${policyNumber ? policyNumber : quotationNumber}`,
      tag: {
        name: "Pending",
        color: "#F68B1F",
      },
  ...
  return (
    <>
      <div className="main-container">
        <QuoteSuccessPage {...QuotationPending} />
      </div>
      {/* <WithoutApproxFooter /> */}
      <WithoutApproxPolicyFooter />
    </>
  );
}
