...
export default function PaymentSuccessMotor() {
  ...
  const agentCodeFromPageNew = useSelector(state => (state?.motorQuotation?.motorPolicyResponse?.data?.agentCode))
  ...
  const printPDF = async () => {
    ...
    let formData = {
      agentCode: login_user_data?.channelCode?.channelCode === "CH1"
        ? agentCodeFromPageNew : login_user_data?.channelCode?.channelCode === "CH1" ? agentCodeFromPage 
        : agentCode,
