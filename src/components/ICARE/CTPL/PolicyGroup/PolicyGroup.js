...
const PolicyGroup = () => {
  ...
  const onchangetoBack = () => {
    history.push("/vehicle-information");
  };
  const onchange = (val) => {
    setSubAgent(val);
  };
  ...
  useEffect(() => {
    ...
    let agentCode = null;
    if (channelCode?.channelCode === "CH1" && qoutationFormData?.quotation_group_info?.agentCode) {
      agentCode = qoutationFormData?.quotation_group_info?.agentCode;
      getAgentList(
        qoutationFormData?.quotation_group_info?.commercial_structure,
        "formUseEffect2"
      );
      setSelectedAgentCode(qoutationFormData?.quotation_group_info?.agentCode);
    } else {
    ...
    setAgent(qoutationFormData?.quotation_group_info?.agentCode ? qoutationFormData?.quotation_group_info?.agentCode : "");
    setAgentName(qoutationFormData?.quotation_group_info?.agentCodeName ? qoutationFormData?.quotation_group_info?.agentCodeName : "");
    ...
    form.setFieldsValue({
      ...
      agent: qoutationFormData?.quotation_group_info?.agentCode ? qoutationFormData?.quotation_group_info?.agentCode : "",
      agentName: qoutationFormData?.quotation_group_info?.agentCodeName ? qoutationFormData?.quotation_group_info?.agentCodeName : "",
  ...
  const onFinishPolicyGroup = async (fomData) => {
    ...
    const groupPolicyInfoRequestBody = {
      quotation_group_info: {
        ...
        agentCode: channelCode?.channelCode === "CH1" ? agent : getAgentCode,
        agentCodeName: channelCode?.channelCode === "CH1" ? agentName : null
      },
    };
    if (selectedSubAgent) {
    ...
    // if (channelCode?.channelCode === "CH1") {
    //   groupPolicyInfoRequestBody?.agentCode = ;
