...
import { Row, Col, Button, Radio, Form, Input, Select, message, Image } from "antd";
import { ArrowLeftOutlined, ArrowRightOutlined } from "@ant-design/icons";
import FullPageLoader from "../../../FullPageLoader/FullPageLoader";
import PolicySelection from "../../TRAVEL/PolicySelection/PolicySelection";
import axiosRequest from "../../../../axios-request/request.methods";
import StepTravelOne from "../StepBarTravel/StepTravelOne/StepTravelOne";
import { useDispatch,useSelector } from "react-redux";
import { stoageGetter } from '../../../../helpers';
import { setTravelInfo, setTravelPolicyDetails, setTravelUserDetails, setQuotationDetails } from "../../../../store/actions/travel"
...
export default function QuoteCode() {
  ...
  const saveDataToRedux = (data) => {
    const travelInfo = {
      ...
      travelProductName: data?.travel_information?.travelData?.travelProductName,
      ...
      countries: [
        {
          ...
          NOM_VERNACULO: data?.travel_information?.travelData?.countries[0]?.type,
        }
      ],
  ...
  const QuoteNext = async () => {
    if (value === "no") {
      if (travelType === "Group") {
        history.push("/travel-bulkUpload");
        return;
      ...
      setOpenloader(true);
      let result = await axiosRequest.get(`user/getQuotation/${loginUserID}?quotationNumber=${documentCode}`, { secure: true, });
      setOpenloader(false);

      if (result.statusCode === -1 && result?.data[0][0]?.oonaQuotationResponse) {
        if (result?.data[0][0].oonaQuotationResponse.technicalControls.length > 0) {
            ...
            <div className="right-sides">
              ...
              <Row gutter={16}>
                <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                  <Button className={`yes ${value === 'yes' ? 'yes-active' : ''}`} onClick={handleButtonClick}>
                    {" "}
                    <CheckIcon
                ...
                <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                  <Button className={`yes ${value === 'no' ? 'yes-active' : ''}`} onClick={handleNoClick}>
                    {" "}
                    <CloseIcon
                  ...
                  <Row style={{ marginTop: "20px" }}>
                    <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                      ...
                      >
                        ...
                        />
                        {quotNumErr && (
                          <div className="error-message" style={{ color: 'red' }}>{quotNumErr}</div>
                        )}
                      </Form.Item>
                    </Col>
                  </Row>
