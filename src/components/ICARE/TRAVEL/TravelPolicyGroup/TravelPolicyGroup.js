...
const TravelPolicyGroup = () => {
  ...
  console.log("travelPolicyData", travelPolicyData);
  ...
  const onFinishPolicyGroup = (formData) => {
    console.log("On finish Console",formData,policyDetails,optionList)
  ...
  const onChangeHandler = (val, name) => {
    console.log("onChangeHandler---", val, name);
  ...
  const prepopulateData = async(data) => {
    ...
    if(data?.commercial?.value && data?.commercial?.label && optionList?.commercial?.length === 0){
    ...
    if(data?.commercial?.value && !policyDetails.commercial){
      ...
      setPolicyDetailsFn("commercial", data?.commercial);
    }

    if(data?.commercial?.label && !policyDetails.commercial){
      setPolicyDetailsFn("commercial", data?.commercial?.label);
    }else{
      setPolicyDetailsFn("commercialName", data?.commercialName);
    }
    let agentCode = null;
    ...
    // if(agentCode && channelCode?.channelCode === "CH1" && optionList?.agent?.length === 0){
    //   await getAgentData(data?.commercial?.label)
    // }

    if(agentCode && channelCode?.channelCode === "CH1" && !policyDetails?.agent){
      console.log("shubham----", agentCode);
