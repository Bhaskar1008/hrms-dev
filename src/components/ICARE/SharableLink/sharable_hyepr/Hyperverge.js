...
const Hyperverge = ({ handleCustomerDetails, handleDocType, handleIdNumber, checkIdAvailable, documentId, hintText, handleHyperResponse }) => {
    ...
    useEffect(() => {
        if (hyperData?.currentLOB == "CTPL") {
            ...
            if ((documentType && documentCode) || (documentType && documentCode && hyperDocImg && hypervergeDocName)) {
    ...
    const onBoardHandler = async (KYCResult) => {
        ...
        // if (!documentType && checkBtnTriggered == "yes") { // OLD CODE
        //   message.error("Please select ID Type");
        //   return;
        // }
        ...
        window.HyperKYCModule.launch(hyperKycConfigs, (ResultsHandler) => {
            ...
            if (ResultsHandler.status == "auto_approved") {
                ...
                if (checkBtnTriggered == "no") {
                    prepopulateHyperData(documentType, documentCode, ResultsHandler?.details?.imagePath, selectedName, false);
                } else {
                    prepopulateHyperData(documentType, documentCode, ResultsHandler?.details?.imagePath, selectedName, true);
                }
                //   if (checkBtnTriggered == "no") {
                            ...
                            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                                ...
                                >
                                    <Select
                                        ...
                                        size="large"
                                        options={validIds}
                                        // disabled={isUserValidate || checkBtnTriggered == "no"} //OLD CODE
                                        value={documentType}
                            ...
                            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                                ...
                                >
                                    <Input
                                        ...
                                        name="documentCode"
                                        value={documentCode}
                                        // disabled={isUserValidate || (checkBtnTriggered === 'no' && !documentCode)} //OLD CODE
