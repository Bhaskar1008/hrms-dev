...
export default function PaymentSuccessMotor() {
  ...
  const PolicyNumber = useSelector(
    (state) =>
      state?.motorQuotation?.motorPolicyResponse?.data?.oonaPolicyResponse
        ?.policyNumber
  );
  const ReferanceNumber = useSelector(
    (state) =>
      state?.motorQuotation?.motorPolicyResponse?.data?.oonaPolicyResponse
        ?.referenceNumber
  );
  const agentCodeFromPage = useSelector(
    (state) => state?.motorQuotation?.motorQutotionSucess?.data?.agentCode
  );
  const agentCodeFromPageNew = useSelector(
    (state) => state?.motorQuotation?.motorPolicyResponse?.data?.agentCode
  );

  console.log("agentCodeFromPage==>", agentCodeFromPage, agentCodeFromPageNew);
  ...
  const printPDF = async () => {
    let formData = {
      ...
      //   ? agentCodeFromPage
      //   : agentCode,
      agentCode: agentCodeFromPage ? agentCodeFromPage : agentCodeFromPageNew,
