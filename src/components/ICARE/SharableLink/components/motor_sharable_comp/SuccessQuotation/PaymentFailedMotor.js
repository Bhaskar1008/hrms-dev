...
export default function PaymentFailedMotor(props) {
  ...
  let altenatePolicyNumber = dataFromPreviousPage?.oonaPolicyResponse?.policyNumber
  ...
  console.log("policyNumberFromMaya", policyNumberFromMaya);
  ...
  const policyPaymentFail = {
    ...
    section2: {
      title: `Policy #${policyNumberFromMaya ? policyNumberFromMaya : altenatePolicyNumber}`,
  ...
  return (
    <>
      <div className="main-container">
        <QuoteSuccessPage
          {...policyPaymentFail}
          onClick={() => paymentFailed(PolicyId ? PolicyId : PolicyID_maya)}
        />
      </div>
      {/* <WithoutApproxFooter /> */}
      <PolicyFooter />
