...
export default function PolicySuccessMotor() {
  ...
  const GetPaymentmethods = async () => {
    ...
    // return
    // setPopupOpen(true);
    let Response = await axiosRequest.get(`user/payment/paymentMethods?product=MOTOR&persona=CLIENT`)
    // let Response = await axiosRequest.get(`user/payment/paymentMethods?product=motor&isAgent=false`)
  ...
  const handleclick = async (event) => {
    ...
    if (OnlyOnnaResponse?.hasOwnProperty("oonaPolicyResponse") === true) {
      formdata = {
        ...
        requestReferenceNumber: OnlyOnnaResponse?.oonaPolicyResponse?.referenceNumber, // policy refrence number
        // ipAddress: "192.168.1.1",
        cancelLink: Apiconfig?.ProjectLink + '/customer-motor-payment-pay-now',
        failureLink: Apiconfig?.ProjectLink + `/customer-motor-payment-return-url?Policy_Number=${MotorPolicy?.oonaPolicyResponse?.policyNumber}`,
        firstName: MotorPolicy?.motor_comprehensive?.policyHolder?.firstName,
        // middleName: MotorPolicy?.motor_comprehensive?.policyHolder?.middleName,
        lastName: MotorPolicy?.motor_comprehensive?.policyHolder?.lastName,
        ...
        amount: MotorPolicy?.oonaPolicyResponse?.paymentBreakdown?.grossPrem,
        paymentMethod: event.paymentMethod ? event.paymentMethod : "",
        successLink: Apiconfig?.ProjectLink + `/customer-motor-payment-return-url?Policy_Number=${MotorPolicy?.oonaPolicyResponse?.policyNumber}`,
        paymentLink: Apiconfig?.ProjectLink + '/customer-motor-cypher-payment/',
        paymentText: event.paymentText,
        responseUrl: Apiconfig?.ProjectLink + "/customer-motor-payment-return-url", //return url
        currency: "PHP",
      ...
      if (event.paymentText === "Payment Link") {
        if (Paymentresponse?.statusCode === -1) {
          ...
          history.push({
            ...
            pathname: "/customer-motor-customer-payment-link",
            state: {
              data: []
            }
          })
        ...
        if (Paymentresponse?.statusCode === -1) {
          message.success(Paymentresponse?.data?.message);
          history.push({
            pathname: "/customer-motor-policy-customer-pay",
            state: {
              data: []
            }
          })
