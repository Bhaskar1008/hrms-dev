          ...
          if (response?.data?.statusCode === -1) {
            if (response?.data?.data?.data?.body?.responseCode === "MA001") {
              ...
              if (response?.data?.data?.data?.isCocafAuthenticated === false && response?.data?.data?.data?.isCocafRequired === true) {
                ...
                history.push({
                  pathname: "/customer-ctpl-payment-success-cypher",
                  state: {
                    data: response?.data?.data?.data,
                    cypher: finalCypher
                  }
              ...
              history.push({
                pathname: "/customer-ctpl-payment-failed-cypher",
                state: {
                  data: response?.data?.data?.policyData ? response?.data?.data?.policyData : response?.data?.data?.data,
                  cypher: finalCypher
                }
              })
            }
