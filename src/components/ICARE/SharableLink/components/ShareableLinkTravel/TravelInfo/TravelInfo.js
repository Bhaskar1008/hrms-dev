...
export default function TravelInfo() {
  ...
  const onClickNext = async () => {
    ...
    const formData = {
      ...
      hazardousSports: isHazardousSports
        ? travelForm.sportsParticipation
        : "false",
  ...
  const prepopulateData = async (data) => {
    if (tripInfo) {
      ...
      if (data?.hazardousSports == "false") {
        setIsHazardousSports(false);
      } else {
        setIsHazardousSports(true);
      }
      if (data?.sportsEquipment == "false") {
      ...
      if (data?.travelType && data?.travelPack && ageRangeList.length === 0) {
        ...
        const selectedMaxAge = ageRange?.filter(
          (item) => item.payloadData == data?.ageOfEldest
        )[0];
        setTravelFormVal("ageOfEldest", selectedMaxAge?.value);
        form.setFieldsValue({
  ...
  const getCurrencyFn = async (_dest, _package) => {
    ...
    if (res.statusCode === -1) {
      ...
      console.log(_dest, _package, "_package")
    }
    setShowScreenLoader(false);
  };
                        ...
                        return (
                          <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                            ...
                            <Button
                              ...
                              className={`destination-btn ${travelForm.destination === item.code
                ...
                <Row gutter={16}>
                  <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                    <Form.Item
                      ...
                      className="form-item-name la formFill"
                      name="ageOfEldest"
                      label="Age Range of Eldest Insured Traveler"
                      rules={[
                        {
                          required: true,
                    ...
                    >
                      <Select
                        ...
                        onChange={(val) => setTravelFormVal("ageOfEldest", val)}
                      ></Select>
                    </Form.Item>
                  </Col>
                </Row>
