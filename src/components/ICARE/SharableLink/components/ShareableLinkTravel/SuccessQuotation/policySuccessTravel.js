...
const policyPaymentFail = {
  section1: {
    ...
    img: "paymentFail",
    subTitleForDown:
      "Please try again later or contact your designated Sales Representative for assistance.",
    share: {
      show: false,
    },
  },
...
export default function SuccessQuotation() {
  ...
  const getTotalValue = () => {
    return tripData?.travelPolicyQuotationData?.oonaPolicyResponse
  ...
  console.log("qoutationFormData", qoutationFormData);
  ...
  const GetPaymentmethods = async () => {
    ...
    let Response = await axiosRequest.get(
      `user/payment/paymentMethods?product=TRAVEL&persona=CLIENT`
    );
    ...
    if (Response?.statusCode === -1) {
      if (Response?.data?.status === 200) {
        if (Response?.data?.data?.length === 1) {
          ...
          let eventdata = {
            paymentText: Response?.data?.data[0]?.paymentText,
  ...
  const Paymnetgatewayredirect = async (event) => {
    ...
    if (CTPLPolicy?.travelUserDetailedInfo === null) {
      let TravelDatafromlisting =
        CTPLPolicy?.travelPolicyQuotationData?.travel_information?.policyHolder;
      ...
      formdata = {
        ...
        requestReferenceNumber:
          CTPLPolicy?.travelPolicyQuotationData?.oonaPolicyResponse
            ?.referenceNumber, // policy refrence number
        cancelLink: Apiconfig?.ProjectLink + "/customer/travel-policy-sucess",
        failureLink:
          Apiconfig?.ProjectLink +
          `/customer/travel-payment-return-url?Policy_Number=${CTPLPolicy?.travelPolicyQuotationData?.policyNumber}`,
        firstName: TravelDatafromlisting?.firstName,
        middleName: TravelDatafromlisting?.middleName,
        lastName: TravelDatafromlisting?.lastName,
        email: TravelDatafromlisting?.emailAddress,
        ...
        paymentText: event.paymentText,
        paymentLink:
          Apiconfig?.ProjectLink + "/customer/travel-cypher-payment/",
        successLink:
          Apiconfig?.ProjectLink +
          `/customer/travel-payment-return-url?Policy_Number=${CTPLPolicy?.travelPolicyQuotationData?.policyNumber}`,
        responseUrl:
          Apiconfig?.ProjectLink + "/customer/travel-payment-return-url", //return url
        currency: "PHP",
      ...
      formdata = {
        ...
        failureLink:
          ...
          `/customer/travel-payment-return-url?Policy_Number=${CTPLPolicy?.travelPolicyQuotationData?.policyNumber}`,
        firstName: CTPLPolicy?.travelUserDetailedInfo?.firstName
          ? CTPLPolicy?.travelUserDetailedInfo?.firstName
          : CTPLPolicy?.travelPolicyQuotationData?.travel_information
            ?.policyHolder?.firstName,
        // middleName: CTPLPolicy?.travelUserDetailedInfo?.middleName,
        lastName: CTPLPolicy?.travelUserDetailedInfo?.lastName ? CTPLPolicy?.travelUserDetailedInfo?.lastName : CTPLPolicy?.travelPolicyQuotationData?.travel_information
          ?.policyHolder?.lastName,
        email: CTPLPolicy?.travelUserDetailedInfo?.emailAddress ? CTPLPolicy?.travelUserDetailedInfo?.emailAddress : CTPLPolicy?.travelPolicyQuotationData?.travel_information
          ?.policyHolder?.emailAddress,
        phone: CTPLPolicy?.travelUserDetailedInfo?.mobileNumber ? CTPLPolicy?.travelUserDetailedInfo?.mobileNumber : CTPLPolicy?.travelPolicyQuotationData?.travel_information
    ...
    if (
      event.paymentText === "CC" ||
      event.paymentText === "GC" ||
      event.paymentText === "Card / E-wallet"
    ) {
      ...
      if (event.paymentText === "Payment Link") {
        if (Paymentresponse?.statusCode === -1) {
          ...
          history.push({
            pathname: "/customer/travel-customer-payment-link",
            state: {
              data: [],
            },
          });
        ...
        if (Paymentresponse?.statusCode === -1) {
          message.success(Paymentresponse?.data?.message);
          history.push({
            pathname: "/customer/travel-policy-customer-pay",
            state: {
              data: [],
            },
          });
  ...
  const handleclick = (event) => {
    if (
      event.paymentText === "CREDIT/DEBIT CARD" ||
      event.paymentText === "GCASH"
    ) {
    ...
    section2: {
      title: (
        <div style={{ fontWeight: 500, fontSize: "17px" }}>
          Payment Reference #
          {
            tripData?.travelPolicyQuotationData?.oonaPolicyResponse
              ?.referenceNumber
          }
        </div>
      ),
