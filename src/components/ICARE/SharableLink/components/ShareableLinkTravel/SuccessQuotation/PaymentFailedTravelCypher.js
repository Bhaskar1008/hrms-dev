...
export default function PaymentFailedTravelCypher(props) {
  ...
  console.log("Props Data=====>>>, finalCypher",dataFromPreviousPage, finalCypher)
  ...
  const paymentFailed = () => {
    history.push("/travel-cypher-payment/" + finalCypher)
    // setopenloader(true)
  ...
  const successPage = {
    section1: {
      ...
      subTitle: `Quotation No. ${policyFormData}`,
      img: "successImg",
      share: {
        show: true,
      },
  ...
  const policyPaymentFail = {
    ...
    section2: {
      ...
      subTitle: `Payment Reference #${RefrenceNumber}`,
      // ${policyReferance}
      data: data,
      displayDetails: false,
      successBtn: {
