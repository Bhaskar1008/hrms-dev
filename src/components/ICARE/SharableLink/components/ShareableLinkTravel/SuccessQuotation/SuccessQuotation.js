...
export default function SuccessQuotation() {
  ...
  const agentCodeForsales = useSelector((state) => state?.JWTDecrypt?.JWTDecryptData?.formData?.tokenData?.agentCode);
  console.log(agentCodeForsales, "agentCodeForsales")
  ...
  console.log("tripDataQuotation===========>", tripData);
  const data = [];
  ...
  const getTotalValue = () => {
    return tripData?.travelQuotationDetails?.paymentBreakdown?.converted;
  };
  ...
  async function downloadQuotePdf() {
    setopenloader(true)
    const formData = {
      agentCode: agentCodeForsales ? agentCodeForsales : getAgentCode,
      agentCopy: true,
      // endoresmentNumber: "0",
      isPayment: false,
      isDownloadable: true,
