...
export default function PolicySuccessCTPL() {
  ...
  console.log("paymentUrlllll--", CTPLPolicyData?.oonaPolicyResponse?.policyNumber);
  ...
  const GetPaymentmethods = async () => {
    ...
    // let Response = await axiosRequest.get(`user/payment/paymentMethods?product=ctpl&isAgent=false`)
    let Response = await axiosRequest.get(`user/payment/paymentMethods?product=CTPL&persona=CLIENT`)
  ...
  const handleclick = async (event) => {
    ...
    let formdata = {
      ...
      cancelLink: Apiconfig?.ProjectLink + "/customer-CTPL-policy-sucess",
      // failureLink: Apiconfig?.ProjectLink + "/CTPL-payment-failed/Policy_Number:" + CTPLPolicyData?.oonaPolicyResponse?.policyNumber,
      failureLink: Apiconfig?.ProjectLink + `/customer-ctpl-payment-return-url?Policy_Number=${CTPLPolicyData?.oonaPolicyResponse?.policyNumber}`,
      ...
      successLink: Apiconfig?.ProjectLink + `/customer-ctpl-payment-return-url?Policy_Number=${CTPLPolicyData?.oonaPolicyResponse?.policyNumber}`,
      paymentLink: Apiconfig?.ProjectLink + `/customer-ctpl-cypher-payment/`,
      paymentText: event.paymentText,
      responseUrl: Apiconfig?.ProjectLink + "/customer-ctpl-payment-return-url", //return url
      currency: "PHP",
    ...
    if (event.paymentText === 'CC' || event.paymentText === 'GC' || event.paymentText === 'Card / E-wallet') {
      ...
      if (event.paymentText === "Payment Link") {
        if (Paymentresponse?.statusCode === -1) {
          ...
          history.push({
            pathname: "/customer-ctpl-customer-payment-link",
            state: {
              data: []
            }
          })
        ...
        if (Paymentresponse?.statusCode === -1) {
          message.success(Paymentresponse?.data?.message);
          history.push({
            pathname: "/customer-ctpl-payment-success",
            state: {
              data: []
            }
          })
