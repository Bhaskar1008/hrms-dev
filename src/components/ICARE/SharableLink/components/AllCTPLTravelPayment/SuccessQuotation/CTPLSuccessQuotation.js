...
export default function CTPLSuccessQuotation() {
  ...
  const AgentCode = GetAgentCodeFromJWT?.userData?.agentCode
  const commercialStructure = GetAgentCodeFromJWT?.tokenData;
  const userIdData = GetAgentCodeFromJWT?.userData._id
  console.log("GetAgentCodeFromJWT?.channelCode", userIdData);
  ...
  let createFormData = { 
    ...
    leadId: LeadId ? LeadId : userIdData,
  ...
  async function downloadQuotePdf() {
    ...
    const formData = {
      agentCode:
      GetAgentCodeFromJWT?.channelCode === "CH1"
          ? AgentCodeFromSelect
          : AgentCode,
