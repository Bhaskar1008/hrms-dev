...
const OTPInput = forwardRef((props,ref) => {
    ...
    useEffect(() => {
        ...
        inputRefs.current = Array(props.length).fill().map((_, index) => inputRefs.current[index]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    const handleInputPaste = (e) => {
      e.preventDefault();
      const pastedData = e.clipboardData.getData('text/plain').slice(0, props.length);
      const otpValues = Array(props.length).fill('').map((_, index) => pastedData[index] || '');
      setOTP(otpValues);
      props.onChange(otpValues.join('')); // Pass the updated OTP value to the parent component
