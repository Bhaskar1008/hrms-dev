...
const CustomerInformation = () => {
  ...
  // zipcode1?.COD_POSTAL otpLoginMobileNumber
  ...
  useEffect(() => {
    ...
    // setMobile(otpLoginMobileNumber);
    setBirthplace(
    ...
    setIsDisable({
      ...
      // mobileDi: otpLoginMobileNumber?.mobile
    });
  }, []);
  ...
  const onFinishCustomer = (formData) => {
    ...
    history.push("/customer-confirm-details");
  };
  ...
  const handleUpload = async (selectFile, name) => {
    ...
    if (sigName) {
      ...
      setLoader(true);
      await axiosRequest
        .post("customer/document/upload", data)
        .then((res) => {
          setLoader(false);
          if (res.statusCode == -1) {
            setShowMsg(true);
                ...
                <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                  ...
                  >
                    <Input
                      ...
                      // disabled={isDisable.mobile && isIdAvailable && isDisable.mobileDi}
                      disabled
